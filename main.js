...
const BUILD_LOOP = (CARDS, SPACING) => {
	...
	for (i = 0; i < L; i++) {
		...
		RAW.fromTo(
			...
			.fromTo(item, { xPercent: 250 }, { xPercent: -250, duration: 1, ease: 'none', immediateRender: false }, time);
...
TRIGGER = ScrollTrigger.create({
	...
	onUpdate: self => {
		if (self.progress === 1 && self.direction > 0 && !self.wrapping) {
			...
			SCRUB.vars.totalTime = CARD_SNAP((iteration + self.progress) * LOOP.duration());
